package com.ordjoy.util;

public final class JspPageConst {

    public static final String SEPARATOR = "/";
    public static final String USER_MAIN_PAGE = "user";
    public static final String GREETING_PAGE = "greeting";
    public static final String FEATURES_PAGE = "features";
    public static final String ABOUT_PAGE = "about";
    public static final String ACCOUNT_SETTINGS_PAGE = "account";
    public static final String ALBUMS_PAGE = "albums";
    public static final String MIXES_PAGE = "mixes";
    public static final String TRACK_PAGE = "tracks";
    public static final String ORDER_PAGE = "makeOrder";
    public static final String UPDATE_USER_SETTINGS = "updateAccountForm";
    public static final String ADMIN_MAIN_PAGE = "admin";
    public static final String ADMIN_DASHBOARD_PAGE = "dashboard";
    public static final String ADD_ADMIN_FORM_PAGE = "addAdminForm";
    public static final String ADD_MIX_FORM_PAGE = "addMixForm";
    public static final String ADD_ALBUM_FORM_PAGE = "addAlbumForm";
    public static final String ADD_TRACK_FORM_PAGE = "addTrackForm";
    public static final String ALL_USERS_PAGE = "users";
    public static final String LOGIN_PAGE = "login";
    public static final String REGISTER_PAGE = "register";
    public static final String ERROR_PAGE = "error";
    public static final String LOGIN_PAGE_FOR_FILTER = "/login.jsp";
    public static final String THANKS_FOR_ORDER_PAGE = "thanksForOrder";
    public static final String ALBUM_REVIEW_FORM_PAGE = "addAlbumReviewForm";
    public static final String TRACK_REVIEW_FORM_PAGE = "addTrackReviewForm";
    public static final String MIX_REVIEW_FORM_PAGE = "addMixReviewForm";

    private JspPageConst() {
        throw new UnsupportedOperationException();
    }
}